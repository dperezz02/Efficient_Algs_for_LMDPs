...
if __name__ == "__main__":
    ...
    start_time = time.time()
    Q_est, est_policy, lengths, throughputs = Qlearning_training(qlearning, opt_lengths, n_steps=int(1e5))
    print("--- %s minutes and %s seconds ---" % (int((time.time() - start_time)/60), int((time.time() - start_time) % 60)))
    ...
    start_time = time.time()
    Z_est, z_lengths, z_throughputs = Zlearning_training(zlearning, opt_lengths, n_steps=int(1e5))
    print("--- %s minutes and %s seconds ---" % (int((time.time() - start_time)/60), int((time.time() - start_time) % 60)))
