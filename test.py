...
if __name__ == "__main__":
    ...
    start_time = time.time()
    Q_est, est_policy, lengths, throughputs = Qlearning_training(qlearning, n_steps=int(5e4))
    print("--- %s minutes and %s seconds ---" % (int((time.time() - start_time)/60), int((time.time() - start_time) % 60)))
    ...
    start_time = time.time()
    Z_est, z_lengths, z_throughputs = Zlearning_training(zlearning, n_steps=int(5e4))
    print("--- %s minutes and %s seconds ---" % (int((time.time() - start_time)/60), int((time.time() - start_time) % 60)))
    ...
    print("Total embedding error: ", np.sum(V-Q2.max(axis=1)))

    # Q-learning Embedded MDP
    print("Q-learning training...")
    qlearning = QLearning(mdp_minigrid, gamma=gamma, learning_rate=1, epsilon_min = 0)
    start_time = time.time()
    Q_est, est_policy, lengths, throughputs = Qlearning_training(qlearning, n_steps=int(5e4))
    print("--- %s minutes and %s seconds ---" % (int((time.time() - start_time)/60), int((time.time() - start_time) % 60)))
